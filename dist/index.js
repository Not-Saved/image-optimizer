...
var ImageOptimizerCache = class {
  ...
  constructor({ distDir }) {
    this.cacheDir = (0, import_path.join)(distDir, "cache", "images");
  }
  async get(cacheKey) {
    try {
      const cacheDir = (0, import_path.join)(this.cacheDir, cacheKey);
      const files = await import_fs.promises.readdir(cacheDir);
      const now = Date.now();
      for (const file of files) {
        ...
        const buffer = await import_fs.promises.readFile((0, import_path.join)(cacheDir, file));
  ...
  async set(cacheKey, value, {
    ...
    try {
      await writeToCacheDir(
        (0, import_path.join)(this.cacheDir, cacheKey),
        value.extension,
        revalidate,
        expireAt,
        value.buffer,
...
async function writeToCacheDir(dir, extension, maxAge, expireAt, buffer, etag, upstreamEtag) {
  const filename = (0, import_path.join)(
    dir,
    `${maxAge}.${expireAt}.${etag}.${upstreamEtag}.${extension}`
  );
  await import_fs.promises.rm(dir, { recursive: true, force: true }).catch(() => {
  });
